...
function Run-VMCommandREST{
    ...
    $virtualMachines = (((Invoke-WebRequest -Uri (-join ('https://management.azure.com/subscriptions/',$SubscriptionId,"/providers/Microsoft.Compute/virtualMachines?api-version=2020-06-01")) -Verbose:$false -Method GET -Headers @{ Authorization ="Bearer $managementToken"} -UseBasicParsing).Content) | ConvertFrom-Json).value
    ...
    if($targetVMArg -ne '')
    {
        ...
        {
            if($vm.name -eq $targetVMArg){$targetVM = $vm}
        }
        if($targetVM -eq ''){
            Write-Output "Could not find target VM, EXITING"
            return
        }
        
    }
    else{$targetVM = $virtualMachines | out-gridview -Title "Select target VM" -PassThru}
